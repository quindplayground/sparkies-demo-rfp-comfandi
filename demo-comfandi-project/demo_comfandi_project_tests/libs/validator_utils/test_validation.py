...
from demo_comfandi_project.libs.validator_utils.parameters_validation import (
...
def test_valid_parameters_with_default_validators():
    with patch(
        "demo_comfandi_project.libs.validator_utils.parameters_validation.get_vars_resource"
    ) as mock_get_vars:
        mock_vars = MagicMock()
...
def test_logging_on_success():
    with patch(
        ...
        "demo_comfandi_project.libs.logging.get_logger"
    ) as mock_get_logger:
